...
;
; Wrapper on plot/oplot, (1) plots sin(y) instead of y, but labelling
; the axes to show y, (2) chooses appropriate ranges in multiples of
; 15s, 30s, 45s or 60s (3) reverses the x-axis, unless xrange is
; specified
;
...
PRO ajs_plot_radec, x, y, overplot=overplot, xrange=xrange, yrange=yrange, $
  ...
  IF ~ keyword_set(overplot) THEN BEGIN 
      ...
      IF xdefault OR ydefault THEN BEGIN
          ;; Get default ticks and ranges (make empty plot on screen)
          my_device = !D.name
          IF my_device NE 'X' THEN $
             set_plot, 'X'
      ...
      IF n_elements(few_x_minors) EQ 0 THEN $
         few_x_minors = 5
      IF n_elements(many_x_minors) EQ 0 THEN $
         many_x_minors = 10
      xminor = n_elements(xtickv) GT 6 ? few_x_minors : many_x_minors
      yminor = 1
